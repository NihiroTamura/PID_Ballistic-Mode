...
void thread_callback() {
  while(1) {
    ...
    if(1) {
      ...
      //  subscribeしたメッセージをBallistc Modeパラメータに格納※board2のみsub[31～34]がパラメータ(Arm Robotの場合)
      for(int i = 0; i < 6; i++){
        if(sub_count == 0){
          sub[31] = change_range_start[0];
          sub[32] = change_range_stop[0];
          sub[33] = speed_range_start[0];
          sub[34] = speed_range_stop[0];
        }

        change_range_start[0] = sub[31];
        change_range_stop[0] = sub[32];
