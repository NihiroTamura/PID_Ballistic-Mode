...
#define AGENT_IP 192, 168, 1, 138 // IP where a micro-ros agent waits
...
void ESO(int index, int roop){
  ...
  //  角速度の推定
  if(lookup_direction == 0){
    dz2[index] = z3[index] + lookup_table0[roop][index] + beta2[index] * ((float)POT_realized[index] - z1[index]) + input_coef[index] * outputADRC[index] - lookup_table0[roop][index];
  }else if(lookup_direction == 1){
    dz2[index] = z3[index] + lookup_table1[roop][index] + beta2[index] * ((float)POT_realized[index] - z1[index]) + input_coef[index] * outputADRC[index] - lookup_table1[roop][index];
  }else{
...
void ADRC(int index, int roop){
  ...
  if(lookup_direction == 0){
    outputADRC_sum[index] = outputADRC[index] - (lookup_table0[roop][index] / input_coef[index]);

  }else if(lookup_direction == 1){
    outputADRC_sum[index] = outputADRC[index] - (lookup_table1[roop][index] / input_coef[index]);
...
void update_lookup_table(int index, int roop){
  ...
  if(lookup_direction == 0){
    lookup_table0[roop][index] = lookup_table0[roop][index] + alpha * z3[index];
    ...
    lookup_table1[roop][index] = lookup_table1[roop][index] + alpha * z3[index];
