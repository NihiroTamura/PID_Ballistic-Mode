...
void ESO(int index){
  ...
  if(sub_count == 0){
    ...
    dz2[index] = z3[index] + u_ff[index] + beta2[index] * ((float)POT_realized[index] - z1[index]) + input_coef[index] * outputADRC[index];
  }
  //  外乱（動特性）の推定
...
void ADRC(int index){
  ...
  if(sub_count == 0){
    ...
    outputADRC[index] = (-z3[index] + kp[index] * errors[index] - kd[index] * z2[index] - u_ff[index]) / input_coef[index];
  }
