...
//--パラメータ関数の係数(A*POT_desired + B*POT_realized = Parameter) --------------------------------------------------------------------
//  パラメータ
float param_func_A[28][6];
float param_func_B[28][6];

//  Ballistic Modeパラメータ構造体
struct Result_BallisticMode {
  int Error_start;
  int Error_stop;
  int Omega_start;
  int Omega_stop;
...
void thread_callback() {
  while(1) {
    ...
    if(1) {
      ...
      for(int i = 0; i < 6; i++){
        if(sub_count == 0){
          sub[8*i+7] = param_func_A[4*i][i];
          sub[8*i+8] = param_func_A[4*i+1][i];
          sub[8*i+9] = param_func_A[4*i+2][i];
          sub[8*i+10] = param_func_A[4*i+3][i];

          sub[8*i+11] = param_func_B[4*i][i];
          sub[8*i+12] = param_func_B[4*i+1][i];
          sub[8*i+13] = param_func_B[4*i+2][i];
          sub[8*i+14] = param_func_B[4*i+3][i];
        }

        param_func_A[4*i][i] = sub[8*i+7];
        param_func_A[4*i+1][i] = sub[8*i+8];
        param_func_A[4*i+2][i] = sub[8*i+9];
        param_func_A[4*i+3][i] = sub[8*i+10];

        param_func_B[4*i][i] = sub[8*i+11];
        param_func_B[4*i+1][i] = sub[8*i+12];
        param_func_B[4*i+2][i] = sub[8*i+13];
        param_func_B[4*i+3][i] = sub[8*i+14];
...
int check_function(int index){
  ...
  //  目標値が変化すればBallistic Modeチェック値を初期化し、Ballistic Modeパラメータを再設定
  if(abs(POT_desired[index] - POT_desired_previous[index]) > 0){
    Ballistic_check[index] = 0;

    //  起動時はPIDのみ
    if(sub_count == 1){
      Result_BallisticMode ballistic = BallisticMode_Param(index);
      change_range_start[index] = ballistic.Error_start;
      change_range_stop[index] = ballistic.Error_stop;
      speed_range_start[index] = ballistic.Omega_start;
      speed_range_stop[index] = ballistic.Omega_stop;
    }
...
//  Ballistic Modeパラメータ計算関数
Result_BallisticMode BallisticMode_Param(int index) {
  Result_BallisticMode param;

  //  パラメータ関数
  float Error = param_func_A[4*index][index] * POT_desired[index] + param_func_B[4*index][index] * POT_realized[index];
  float delta_Error = param_func_A[4*index+1][index] * POT_desired[index] + param_func_B[4*index+1][index] * POT_realized[index];
  float Omega = param_func_A[4*index+2][index] * POT_desired[index] + param_func_B[4*index+2][index] * POT_realized[index];
  float delta_Omega = param_func_A[4*index+3][index] * POT_desired[index] + param_func_B[4*index+3][index] * POT_realized[index];

  //  Ballistic Modeパラメータ計算
  int error_start = Error + (delta_Error / 2);
  int error_stop = Error - (delta_Error / 2);
  int omega_start = Omega + (delta_Omega / 2);
  int omega_stop = Omega - (delta_Omega / 2);

  //  Ballistic Modeパラメータの制約条件(Ballistic Modeパラメータ > 0)
  error_start = max(0, error_start);
  error_stop = max(0, error_stop);
  omega_start = max(0, omega_start);
  omega_stop = max(0, omega_stop);

  param.Error_start = error_start;
  param.Error_stop = error_stop;
  param.Omega_start = omega_start;
  param.Omega_stop = omega_stop;
