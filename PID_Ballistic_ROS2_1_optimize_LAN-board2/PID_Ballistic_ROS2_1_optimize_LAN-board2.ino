...
void thread_callback() {
  while(1) {
    ...
    if(1) {
      ...
      //  subscribeしたメッセージをBallistc Modeパラメータに格納※board2のみsub[55～62]がパラメータ(Arm Robotの場合)
      for(int i = 0; i < 6; i++){
        if(sub_count == 0){
          sub[55] = param_func_A[0][0];
          sub[56] = param_func_A[1][0];
          sub[57] = param_func_A[2][0];
          sub[58] = param_func_A[3][0];

          sub[59] = param_func_B[0][0];
          sub[60] = param_func_B[1][0];
          sub[61] = param_func_B[2][0];
          sub[62] = param_func_B[3][0];
        }

        param_func_A[0][0] = sub[55];
        param_func_A[1][0] = sub[56];
        param_func_A[2][0] = sub[57];
        param_func_A[3][0] = sub[58];

        param_func_B[0][0] = sub[59];
        param_func_B[1][0] = sub[60];
        param_func_B[2][0] = sub[61];
        param_func_B[3][0] = sub[62];
