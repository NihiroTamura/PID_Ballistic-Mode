...
#define TEENSY_IP 192, 168, 1, 88 // IP that the teensy 4.1 will have ※teensyのIPは192.168.[0-3].[0-255]
...
//  継続時間（95%整定時間）
float settle_time[6] = {1.17, 1.17, 1.17, 1.5, 1.17, 1.17};
...
void AddInput(int index) {
  ...
  float s = ((t - t_start) / 1000.0) / settle_time[index];
  s = max(0, min(1.0, s));

  /*特定のみ対応用 4/13の1.線形補間と2.z3の変化確認用
  if(index == 3){
    if(r2 == 700.0 && r1 == 300.0 && dr == 400.0) {
      //outputAdd[index] = 316.68611 * s * s * s -482.53500 * s * s + 165.84889 *s;//A1=8, A2=-14, T=1.5 s1=0.42, s2=0.8 (700)
      //outputAdd[index] = -3.5256 * s * s * s * s * s + 4.6073 * s * s * s * s + 317.16 * s * s * s - 484.6 * s * s + 166.3 *s;
      outputAdd[index] = -42.47 * s * s * s * s * s + 99.83 * s * s * s * s + 237.35 * s * s * s - 458.03 * s * s + 163.32 *s;//t1=0.33, t2=1.19, T=1.5, y1=16.50, y2=-14.0 (700)

    } else if(r2 == 690.0 && r1 == 300.0 && dr == 390.0) {
      //1.線形補間
      outputAdd[index] = -21.82 * s * s * s * s * s + 60.48 * s * s * s * s + 231.59 * s * s * s - 430.94 * s * s + 160.70 *s;//t1=0.3505, t2=1.215, T=1.5, y1=17.14, y2=-11.075 (700と680の線形補間)

    } else if(r2 == 680.0 && r1 == 300.0 && dr == 380.0) {
      //outputAdd[index] = 263.15789 * s * s * s -423.68421 * s * s + 160.52632 *s;//A1=0, A2=-8, T=1.5 s1=0.61, s2=0.8 (680)
      //outputAdd[index] = 1.1925 * s * s * s * s * s - 7.1046 * s * s * s * s + 273.87 * s * s * s - 429.47 * s * s + 161.47 *s;
      outputAdd[index] = -22.28 * s * s * s * s * s + 58.24 * s * s * s * s + 209.60 * s * s * s - 403.67 * s * s + 158.11 *s;//t1=0.371, t2=1.24, T=1.5, y1=17.78, y2=-8.15 (680)
    
    } else if(r2 == 670.0 && r1 == 300.0 && dr == 370.0) {
      //1.線形補間
      outputAdd[index] = 8.44 * s * s * s * s * s - 16.09 * s * s * s * s + 188.70 * s * s * s - 291.16 * s * s + 110.11 *s;//t1=0.3705, t2=1.245, T=1.5, y1=12.225, y2=-5.605 (680と660の線形補間)

    } else if(r2 == 660.0 && r1 == 300.0 && dr == 360.0) {
      //outputAdd[index] = 98.68421 * s * s * s -158.88158 * s * s + 60.19737 *s;//A1=0, A2=-3, T=1.5 s1=0.61, s2=0.8 (660)
      //outputAdd[index] = 0.1047 * s * s * s * s * s - 1.4838 * s * s * s * s + 101.44 * s * s * s - 160.56 * s * s + 60.494 *s;
      outputAdd[index] = 17.86 * s * s * s * s * s -40.08 * s * s * s * s + 127.79 * s * s * s - 166.42 * s * s + 60.85 *s;//t1=0.37, t2=1.25, T=1.5, y1=6.67, y2=-3.06 (660)

      //2.z3の変化確認用
      //outputAdd[index] = -187.32 * s * s * s * s * s + 565.55 * s * s * s * s - 512.60 * s * s * s + 114.13 * s * s + 20.25 *s;//t1=0.481, t2=1.25, T=1.5, y1=6.67, y2=-3.06 (+30% t1のみ変化)
      //outputAdd[index] = 255.88 * s * s * s * s * s - 723.92 * s * s * s * s + 820.75 * s * s * s - 447.68 * s * s + 94.96 *s;//t1=0.259, t2=1.25, T=1.5, y1=6.67, y2=-3.06 (-30% t1のみ変化)
      //outputAdd[index] = 218.87 * s * s * s * s * s - 512.33 * s * s * s * s + 496.16 * s * s * s - 274.02 * s * s + 71.32 *s;//t1=0.37, t2=1.3125, T=1.5, y1=6.67, y2=-3.06 (+5% t2のみ変化)
      //outputAdd[index] = -260.85 * s * s * s * s * s + 596.82 * s * s * s * s - 356.24 * s * s * s - 27.16 * s * s + 47.42 *s;//t1=0.37, t2=1.125, T=1.5, y1=6.67, y2=-3.06 (-10% t2のみ変化)
      //outputAdd[index] = -3.49 * s * s * s * s * s - 9.16 * s * s * s * s + 146.76 * s * s * s - 213.03 * s * s + 78.92 *s;//t1=0.37, t2=1.25, T=1.5, y1=8.671, y2=-3.06 (+30% y1のみ変化)
      //outputAdd[index] = 39.21 * s * s * s * s * s - 71.00 * s * s * s * s + 108.82 * s * s * s - 119.81 * s * s + 42.78 *s;//t1=0.37, t2=1.25, T=1.5, y1=4.669, y2=-3.06 (-30% y1のみ変化)
      //outputAdd[index] = -8.85 * s * s * s * s * s + 2.86 * s * s * s * s + 108.42 * s * s * s - 163.10 * s * s + 60.67 *s;//t1=0.37, t2=1.25, T=1.5, y1=6.67, y2=-2.142 (+30% y2のみ変化)
      //outputAdd[index] = 44.57 * s * s * s * s * s - 83.02 * s * s * s * s + 147.15 * s * s * s - 169.74 * s * s + 61.04 *s;//t1=0.37, t2=1.25, T=1.5, y1=6.67, y2=-3.978 (-30% y2のみ変化)
  ...
  if(index == 3){
    if(r2 == 700.0 && r1 == 300.0 && dr == 400.0) {
      ...
      //圧力差(PWM)35くらい
      //outputAdd[index] = -45.81 * s * s * s * s * s + 70.17 * s * s * s * s + 285.36 * s * s * s - 474.70 * s * s + 164.98 *s;//t1=0.33, t2=1.19, T=1.5, y1=16.50, y2=-14.0 (700)
  ...
  /*色々対応用
  if(index == 3){
    if(690 <= r2 && r2 <= 700.0 && r1 == 300.0 && 390 <= dr && dr <= 400.0) {
      //outputAdd[index] = 316.68611 * s * s * s -482.53500 * s * s + 165.84889 *s;//A1=8, A2=-14, T=1.5 s1=0.42, s2=0.8 (700)
      //outputAdd[index] = -3.5256 * s * s * s * s * s + 4.6073 * s * s * s * s + 317.16 * s * s * s - 484.6 * s * s + 166.3 *s;
      outputAdd[index] = -42.47 * s * s * s * s * s + 99.83 * s * s * s * s + 237.35 * s * s * s - 458.03 * s * s + 163.32 *s;//t1=0.33, t2=1.19, T=1.5, y1=16.50, y2=-14.0 (700)

    } else if(661 <= r2 && r2 <= 689.0 && r1 == 300.0 && 361 <= dr && dr <= 389.0) {
      //outputAdd[index] = 263.15789 * s * s * s -423.68421 * s * s + 160.52632 *s;//A1=0, A2=-8, T=1.5 s1=0.61, s2=0.8 (680)
      //outputAdd[index] = 1.1925 * s * s * s * s * s - 7.1046 * s * s * s * s + 273.87 * s * s * s - 429.47 * s * s + 161.47 *s;
      outputAdd[index] = -22.28 * s * s * s * s * s + 58.24 * s * s * s * s + 209.60 * s * s * s - 403.67 * s * s + 158.11 *s;//t1=0.371, t2=1.24, T=1.5, y1=17.78, y2=-8.15 (680)

    } else if(630 <= r2 && r2 <= 660.0 && r1 == 300.0 && 330 <= dr && dr <= 360.0) {
      //outputAdd[index] = 98.68421 * s * s * s -158.88158 * s * s + 60.19737 *s;//A1=0, A2=-3, T=1.5 s1=0.61, s2=0.8 (660)
      //outputAdd[index] = 0.1047 * s * s * s * s * s - 1.4838 * s * s * s * s + 101.44 * s * s * s - 160.56 * s * s + 60.494 *s;
      outputAdd[index] = 17.86 * s * s * s * s * s -40.08 * s * s * s * s + 127.79 * s * s * s - 166.42 * s * s + 60.85 *s;//t1=0.37, t2=1.25, T=1.5, y1=6.67, y2=-3.06 (660)
